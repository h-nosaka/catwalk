...
func (p *Table) Create() string {
	...
	indexes := []string{}
	sort.Slice(p.Indexes, func(a, b int) bool { return p.Indexes[a].Name < p.Indexes[b].Name })
	for _, item := range p.Indexes {
		indexes = append(indexes, item.Create(p))
	}
	buf.WriteString(fmt.Sprintf("%s\n", strings.Join(indexes, "")))
	relations := []string{}
	sort.Slice(p.Relations, func(a, b int) bool { return p.Relations[a].Name < p.Relations[b].Name })
...
func (p *Table) Diff(src *[]Table) string {
	...
	if dest.Name == "" {
		...
		// 	fmt.Println(base.Diff(p.Create(), dest.Create()))
		// }
		// カラムの差分
		for _, item := range p.Columns {
