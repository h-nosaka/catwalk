...
var PARTS = map[string]string{
	...
	"IndexCreatePrimary":          "ALTER TABLE %s ADD PRIMARY KEY (`%s`);\n",                         // schema.table, columns
	"IndexCreateUnique":           "CREATE UNIQUE INDEX IF NOT EXISTS %s USING BTREE ON %s (`%s`);\n", // index, schema.table, columns
	"IndexCreate":                 "CREATE INDEX IF NOT EXISTS %s USING BTREE ON %s (`%s`);\n",        // index, schema.table, columns
